...
export default function PhotoLightbox({
  ...
  // Tocar el fondo oscuro cierra; tocar la foto o un botón, no.
  const cerrarSiEsFondo = (e: React.MouseEvent) => {
  ...
  return (
    <Superpuesto>
      <div className="fixed inset-0 z-[60] flex flex-col bg-black/92 backdrop-blur-sm" role="dialog" aria-modal="true">
        <div
          className="flex items-center justify-between px-4 pb-3 pt-[max(env(safe-area-inset-top),0.75rem)] text-white"
          onClick={cerrarSiEsFondo}
        >
        ...
        <div className="relative flex flex-1 items-center justify-center overflow-hidden px-3 pb-6" onClick={cerrarSiEsFondo}>
          {photos.length > 1 ? (
            <button
