  ...
  return (
    <div className="rounded-2xl border border-violet-400/30 bg-gradient-to-br from-violet-500/15 via-white/[0.02] to-cyan-500/10 p-4 sm:p-5">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div>
          <p className="text-[11px] uppercase tracking-[0.1em] text-white/50">{t('puntos.tuNivel')}</p>
          <p className="text-lg font-bold text-white">{levelLabel}</p>
        </div>
        <div className="ml-auto text-right">
          <p className="text-2xl font-bold text-cyan-300">{summary.totalPoints}</p>
          <p className="text-xs text-white/50">{t('ranking.puntos')}</p>
        </div>
      </div>
      {summary.nextLevel ? (
        <div className="mt-4">
          ...
          <p className="mt-1.5 text-xs text-white/60">
            {t('puntos.faltanPara')} <span className="font-semibold text-white">{summary.pointsToNext}</span> {t('ranking.puntos')} → {nextLabel}
          </p>
        </div>
      ) : (
...
export function PointsHistoryList({ events, lang, t }: { events: PointsHistoryEntryClient[]; lang: 'es' | 'en'; t: (k: string) => string }) {
  if (events.length === 0) {
    return <p className="text-xs text-white/40">{t('puntos.sinHistorial')}</p>;
  }
  return (
      ...
      {events.map((ev, idx) => {
        ...
        return (
          <div
            ...
            className="flex items-center justify-between gap-2 rounded-xl border border-white/5 bg-white/5 px-3 py-2 text-sm transition-colors duration-200 hover:border-emerald-400/20"
          >
            <span className="flex min-w-0 items-center gap-2">
              <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-emerald-400" />
              <span className="truncate text-white/80">{def ? (lang === 'es' ? def.pastLabelEs : def.pastLabelEn) : ev.eventType}</span>
            </span>
            <span className="flex shrink-0 items-center gap-2">
              <span className="text-[11px] text-white/40">{fmtDate(ev.createdAt, lang)}</span>
        ...
        <div
          ...
          className={`rounded-xl px-3 py-2 text-sm transition-colors duration-200 ${
            key === 'CLOSING_REGISTERED'
              ...
              : 'border border-white/5 bg-white/5 hover:border-emerald-400/25'
          }`}
        >
          <div className="flex items-center justify-between gap-2">
            <span className="min-w-0 truncate text-white/80">{lang === 'es' ? def.labelEs : def.labelEn}</span>
...
export function BreakdownCard({
  ...
  return (
    <div className="group min-w-0 rounded-2xl border border-white/10 bg-gradient-to-br from-emerald-500/[0.06] via-white/[0.02] to-transparent p-4 transition-colors duration-200 hover:border-emerald-400/25">
      <div className="flex min-w-0 items-center justify-between gap-2">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="truncate text-xs uppercase tracking-[0.12em] text-white/50">{label}</p>
      ...
      {dropdown && open ? (
        <div className="mt-3 min-w-0 space-y-1">
          {items ? (
            items.length === 0 ? (
              <p className="text-xs text-white/40">{t('ranking.sinDatos')}</p>
            ) : (
              items.map((item) => (
                <p key={item.id} className="min-w-0 truncate rounded-lg border border-white/5 bg-white/5 px-2 py-1.5 text-xs text-white/70">
                  {item.label}
                </p>
              ))
            )
            ...
            rows.map((row) => (
              <div key={row.label} className="flex min-w-0 items-center justify-between gap-2 text-xs text-white/70">
