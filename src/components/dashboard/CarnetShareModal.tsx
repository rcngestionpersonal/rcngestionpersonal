...
export default function CarnetShareModal({
  ...
  return (
    ...
    >
      <div
        className="max-h-[92vh] w-full max-w-[420px] overflow-y-auto rounded-t-[24px] border border-white/10 bg-[#141722] p-5 sm:rounded-[24px]"
      ...
      >
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-[16px] font-bold text-white">{t('ranking.compartirCarnet')}</h3>
          <button
            onClick={onClose}
        ...
        <div className="mt-4 grid grid-cols-2 gap-1.5 rounded-full border border-white/10 bg-white/[0.03] p-1">
          {(['colegas', 'clientes'] as const).map((opt) => (
            <button
              key={opt}
              onClick={() => setAudience(opt)}
              className={`rounded-full py-2 text-[13px] font-bold transition-colors ${
                audience === opt ? 'bg-[rgba(45,212,191,0.16)] text-[#2dd4bf]' : 'text-[#9296b0] hover:text-white'
              }`}
            >
        ...
        <button
          ...
          disabled={sharing}
          aria-label={t('ranking.compartirCarnet')}
          className="mt-4 flex w-full items-center justify-center gap-2 rounded-[12px] bg-[#2dd4bf] px-4 py-3 text-sm font-bold text-[#04201c] transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
        >
          ↗ {sharing ? t('ranking.carnet.generando') : t('ranking.compartirCarnet')}
        </button>
        {toast ? <p className="mt-2 text-center text-xs text-[#2dd4bf]">{toast}</p> : null}
      </div>
    </div>
  );
}
