// ===========================================================================
// RESERVA DE COMPRAVENTA DE BIEN INMUEBLE — v4-2026-09
//
// ⚠️  PENDIENTE DE REVISIÓN LEGAL  ⚠️  (igual que la v3)
//
// Es la v3 con dos cambios de presentación y ninguno de fondo:
//
//   1. Sale el recuadro de advertencia bajo el título ("Este instrumento
//      constituye una RESERVA y no una promesa de compraventa…"). Repetía lo
//      que ya dice la cláusula de NATURALEZA JURÍDICA, que no cambia.
//   2. La fila "Inmueble" de la ficha resumen muestra la descripción tal como
//      la escribe la cláusula de OBJETO DE LA RESERVA (tercera). Las dos salen
//      de descripcionDelInmueble(), así que no pueden decir cosas distintas.
//
...
export const PLANTILLA_VERSION = 'reserva-compraventa-v4-2026-09';
...
// El inmueble tal como lo identifica la cláusula de objeto. La ficha resumen
// usa esta misma función: si cambia la descripción, cambia en los dos lugares.
export function descripcionDelInmueble(d: DatosDocumento): string {
  const predio = d.campo('predio');
  return `${d.inmueble.descripcion}, ubicado en ${d.inmueble.ubicacion}${predio ? `, predio N.º ${predio}` : ''}. ${d.inmueble.caracteristicas}`
    .replace(/\s+/g, ' ')
    .trim();
}
...
export function construirBloques(d: DatosDocumento): BloqueDocumento[] {
  ...
  return [
...

    {
      ...
      filas: [
        { etiqueta: 'Inmueble', valor: descripcionDelInmueble(d) },
